...
def reward_imitation(
    ...
    lin_vel_z_rew = (
        np.exp(-8.0 * np.square(base_lin_vel[2] - ref_base_lin_vel[2]))
        * w_lin_vel_z
    )
    ...
    ang_vel_z_rew = (
        np.exp(-2.0 * np.square(base_ang_vel[2] - ref_base_ang_vel[2]))
        * w_ang_vel_z
    )
    ...
    joint_vel_rew = np.exp(-0.5 * joint_vel_error) * w_joint_vel

    # Contact pattern matching (bounded [0, weight])
    contact_matches = np.sum(contacts == ref_foot_contacts)
